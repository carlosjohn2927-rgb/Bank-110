...
<div class="goals-grid">
  ...
  <article class="goal-card <?= $is_done ? 'goal-card--done' : '' ?>" style="--accent:<?=html_escape($g['color'] ?: '#1468e5')?>">
    <header>
      ...
      <form method="post" action="<?=site_url('goals/'.$g['id'].'/delete')?>" onsubmit="return confirm('Delete this goal? This cannot be undone.');" style="margin-left:auto">
...
<dialog id="new-goal">
  ...
  <form method="post" action="<?=site_url('goals/create')?>" style="padding:24px">
...
<dialog id="add-<?=$g['id']?>">
  ...
  <form method="post" action="<?=site_url('goals/'.$g['id'].'/contribute')?>" style="padding:24px">
...
<dialog id="withdraw-<?=$g['id']?>">
  ...
  <form method="post" action="<?=site_url('goals/'.$g['id'].'/withdraw')?>" style="padding:24px">
